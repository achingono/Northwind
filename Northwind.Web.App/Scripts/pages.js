...
// This is the object we'll use to maintain UI state
viewModel = {
    Selected: {
        Item: ko.observable({}),
...
String.prototype.format = function (mask, utc) {
    try {
        return (new Date(this.replace(/[T-]/g, ' '))).format(mask, utc)
    }
    catch (err) {
        return '';
    }
...
$(function () {
    ...
    body.on('click', 'a[data-template]', function (e) {
        ...
        if (self.data('item')) {
            ...
            dataItem = ko.dataFor(e.currentTarget);
        }
    ...
    $('#modal a.btn-primary').on('click', function (e) {
        ...
        if (cookieName && cookieName != '') {
            $.cookie(cookieName, ko.mapping.toJSON(viewModel[cookieName]()), { path: '/' });
        }
        ...
        $.ajax({
            ...
            data: ko.mapping.toJS(viewModel.Selected.Item())
        })
